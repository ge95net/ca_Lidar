...
class Explorer(object):
    ...
    def run_k_episodes(self, k, phase, update_memory=False, imitation_learning=False, episode=None,
        ...
        for i in range(k):
            ...
            elif isinstance(info, FarAway):
                faraway += 1
                faraway_cases.append(i)
                faraway_times.append(self.env.time_limit)
            else:
        ...
        faraway_rate = faraway / k
        assert success + collision + timeout +faraway == k

        avg_nav_time = sum(success_times) / len(success_times) if success_times else self.env.time_limit
        extra_info = '' if episode is None else 'in episode {} '.format(episode)
        logging.info('{:<5} {}has success rate: {:.2f}, collision rate: {:.2f},timeout rate: {:.2f},faraway_rate: {:.2f}, nav time: {:.2f}, total reward: {:.4f}'.
                     format(phase.upper(), extra_info, success_rate, collision_rate,timeout_rate,faraway_rate ,avg_nav_time,
